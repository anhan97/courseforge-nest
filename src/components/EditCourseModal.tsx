...
import { X, Plus, AlertCircle, Save, Upload, Globe, Clock, DollarSign, Tag, Users } from 'lucide-react';
import { apiClient, type CreateCourseData, type AdminCourse } from '../lib/api';
...
export function EditCourseModal({ isOpen, onClose, onSuccess, course }: EditCourseModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await apiClient.updateCourse(course.id, formData);
      if (response.success) {
        onSuccess();
        onClose();
      } else {
